...
const proximosEventos: EventCardProps[] = [
    {
        ...
        url: "https://www.instagram.com/tanukiworldfest/?hl=en"
    },
    {
        id: 2,
        date: '19 Julho 2023',
    ...
    return (
        <main className="w-full bg-slate-900 min-h-full flex justify-center">
            <section id="feed" className="w-full max-w-2xl bg-gray-700 min-h-screen flex flex-col gap-10 p-11 rounded-lg mb-8">
                <h1 className="text-3xl text-white font-roboto font-medium">Próximos eventos</h1>

                {proximosEventos.map((evento) => (
                    <a key={evento.id} href={evento.url} target="_blank">
                        <EventCard
                            id={evento.id}
                            img={evento.img}
                            date={evento.date}
